...
class PaperTemplateExporter:
    def export(
        ...
        *,
        ...
        has_citations = self._draft_has_citations(draft_text)
        ...
        if template_spec.kind == TEMPLATE_KIND_SPRINGER_LATEX and bibliography_path and not has_citations:
            status_cb(
                "[TemplateExport] Draft contains no citation markers; using an empty bibliography block instead of references.bib for the current LaTeX draft."
            )
            bibliography_path = ""
        ...
        if template_spec.kind == TEMPLATE_KIND_SPRINGER_LATEX:
            self._sync_springer_assets(context.workspace, template_spec.root)
    ...
    @staticmethod
    def _sync_springer_assets(workspace: Path, template_root: Path) -> None:
        asset_names = (
            "svjour3.cls",
            "svglov3.clo",
            "spbasic.bst",
    ...
    @staticmethod
    def _draft_has_citations(text: str) -> bool:
        patterns = (
